...
***
 ...
 Разработка информационной системы делится на два этапа: проектирование и реализация[3]. Проектирование является начальным уровнем разработки. На данном этапе описываются система и ее элементы, конкретные требования к системе, а также конкретные технические, программные и художественные решения. 
...
***
 ...
 Определение главных действующих лиц и их основных функций в информационной системе реализовано на диаграмме Use Case(см.рисунок 1)[4].
 
![alt text](Screen/UCD.png)
Рисунок 1 - Диаграмма Use Case

 На диаграмме потоков данных (Data-flow diagram), изображено как процессы и хранилища связаны потоками данных(см.рисунок 2)[5].

![alt text](Screen/DFD.png)
Рисунок 2 - Диаграмма потоков данных

 На диаграмме Сущность-Связь (Entity-Relationship diagram) отображены сущности (хранилища) и связи между ними(см.рисунок 3)[6].
...
На основании ER диаграммы для каждой сущности был создан класс с указанием свойств, параметров и типов данных. Листинг  1 - Класс Car.
...
Листинг  2 - Контроллер для класса Car с методами CRUD (create, read, update, delete)[7].
...
~~~      
 ...
 После реализованы контроллеры для остальных сущностей(см.рисунок 5)[8].
 
 ![alt text](Screen/4.png)
 Рисунок 5 - Контроллеры других сущностей
 
Листинг  3 - Репозиторий для класса Car.
...
~~~
 ...
 Далее были созданы хранилища для остальных сущностей(см.рисунок 6). 
...
В качестве тестирования информационной системы была выполнена проверка работы контроллеров в Swagger UI(см.рисунок 7).
![alt text](Screen/22.png)
Рисунок 7 - Проверка контроллеров в Swagger UI

Работа основных методов CRUD на примере сущности Car.

1. С помощью операции Create можно создавать новые записи в таблице(см.рисунок 8).

![alt text](Screen/6.png)
Рисунок 8 - Реализация операции Create

Создание записи о некотором автомобиле(см.рисунок 9).

![alt text](Screen/7.png)
Рисунок 9 - Создание новой записи

Результат создания новой записи(см.рисунок 10).

![alt text](Screen/8.png)
Рисунок 10 - Результат операции Create

2. Метод Read позволяет считать значение записи, выбранной по определенному критерию(см.рисунок 11).

![alt text](Screen/9.png)
Рисунок 11 - Реализация операции Read 

Реализация чтения информации об автомобиле с ID=1(см.рисунок 12).

![alt text](Screen/10.png)
Рисунок 12 - Чтение записи

Результат считывания(см.рисунок 13).

![alt text](Screen/11.png)
Рисунок 13 - Результат операции Read

3. Операция Update позволяет изменить запись(см.рисунок 14).

![alt text](Screen/12.png) 
Рисунок 14 - Реализация операции Update

Изменение марки автомобиля(см.рисунок 15).

![alt text](Screen/13.png)
Рисунок 15 - Редактирование записи

Результат операции Update(см.рисунок 16).

![alt text](Screen/14.png)
Рисунок 16 - Результат операции Update

4. Метод Delete удаляет записи. В случае успешного удаления выводится true, иначе - false(см.рисунок 17).

![alt text](Screen/15.png)
Рисунок 17 - Реализация метода Delete

Удаление записи об автомобиле(см.рисунок 18).

![alt text](Screen/16.png)
Рисунок 18 - Удаление записи

Результат удаления записи(см.рисунок 19).

![alt text](Screen/17.png)
Рисунок 19 - Результат операции Delete
...
[1] Разработка автоматизированной системы по вводу и эксплуатации оборудования в сети АЗС "Газпром". АЗС [Электронный ресурс]. – Режим доступа: https://dspace.susu.ru/xmlui/bitstream/handle/0001.74/24634/2018_532_galkinaem.pdf?sequence=1, свободный (дата доступа: 10.12.2021). 
[2] Автоматические АЗС. ТерминалАЗС [Электронный ресурс]. – Режим доступа: http://azsterminal.ru/articles/avtomaticheskie-azs, свободный (дата доступа: 05.11.2021).
[3] Проектирование и строительство АГЗС. Топливные Модульные Системы [Электронный ресурс]. – Режим доступа: https://topmodul.ru/, свободный (дата доступа: 07.11.2021)`.
[4] Учебное пособие по диаграмме вариантов использования UML. Lucidchart [Электронный ресурс]. – Режим доступа: https://www.lucidchart.com/pages/uml-use-case-diagram, свободный (дата обращения: 08.10.2021).
[5] Что такое диаграмма DFD и как ее создать? Lucidchart [Электронный ресурс]. – Режим доступа: https://www.lucidchart.com/pages/ru/%D0%B4%D0%B8%D0%B0%D0%B3%D1%80%D0%B0%D0%BC%D0%BC%D0%B0-dfd, свободный (дата обращения: 20.10.2021).
[6] Что такое ER-диаграмма и как ее создать? Lucidchart [Электронный ресурс]. – Режим доступа: https://www.lucidchart.com/pages/ru/erd-%D0%B4%D0%B8%D0%B0%D0%B3%D1%80%D0%B0%D0%BC%D0%BC%D0%B0, свободный (дата доступа: 5.11.2021).
[7] CRUD. Википедия [Электронный ресурс]. – Режим доступа: [7] https://ru.wikipedia.org/wiki/CRUD, свободный (дата обращения: 30.11.2021).
[8] Контроллеры и их действия. METANIT.COM [Электронный ресурс]. – Режим доступа: https://metanit.com/sharp/aspnet5/5.1.php, свободный (дата обращения: 30.09.2021).
